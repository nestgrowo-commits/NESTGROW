...
class Command(BaseCommand):
    help = 'Envia todos los datos de SQLite local a Railway PostgreSQL'
    ...
    def handle(self, *args, **options):
        ...
        try:
            for app in apps_to_dump:
                self.stdout.write(f'Sincronizando {app}...')
                buf = io.StringIO()
                try:
                    call_command(
                        'dumpdata', app,
                ...
                try:
                    call_command('loaddata', tmp, database='railway', verbosity=0)
            ...
            self.stdout.write(self.style.SUCCESS('Sincronizacion local -> Railway completada'))
        except Exception as e:
